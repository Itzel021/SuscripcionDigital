...
$app->get('/sse', function (Request $request, Response $response) {
    ...
    // Leer el archivo temporal donde se registran los nuevos productos
    $file = __DIR__ . '/productos_nuevos.json'; // Usar un archivo para almacenar productos temporalmente
    if (file_exists($file)) {
        $data = file_get_contents($file);
        if (!empty($data)) {
            // Enviar los datos como un evento SSE
            $response->getBody()->write("event: nuevoProducto\n");
            $response->getBody()->write("data: {$data}\n\n");

            // Vaciar el archivo después de enviar los datos
...
$app->post('/webhooks/productos', function (Request $request, Response $response) use ($firebase) {
    ...
    try {
        ...
        foreach ($correos as $email) {
         ...
         // Guardar el producto en el archivo temporal para SSE
         $file = __DIR__ . '/productos_nuevos.json';
         $nuevoProducto = [
         ...
         file_put_contents($file, json_encode($nuevoProducto));
